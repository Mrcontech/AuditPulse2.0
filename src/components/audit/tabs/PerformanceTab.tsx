...
export const PerformanceTab = ({ results }: { results: any }) => {
    ...
    const metrics = [
        { key: 'lcp', label: 'LCP', fullLabel: 'Largest Contentful Paint', value: results.lcp_desktop, unit: 's' },
        { key: 'cls', label: 'CLS', fullLabel: 'Cumulative Layout Shift', value: results.cls_desktop, unit: '' },
        { key: 'inp', label: 'INP', fullLabel: 'Interaction to Next Paint', value: results.inp_desktop, unit: 'ms' }
    ];

    // Score breakdown data
    const scoreData = [
        { name: 'Performance', value: results.performance_score || 0 },
        { name: 'Accessibility', value: results.accessibility_score || 85 },
        { name: 'Best Practices', value: results.best_practices_score || 78 }
    ];
                    ...
                    return (
                        <div key={i} className="bg-card border border-white/[0.06] rounded-lg p-5">
                            <div className="flex items-center justify-between mb-2">
                                <span className="text-xs text-muted-foreground">{m.fullLabel}</span>
                            </div>
